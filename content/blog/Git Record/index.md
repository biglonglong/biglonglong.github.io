...
简单来说，git构建多分支版本树（v1.0 -> v1.1 -> … -> v1.9 -> <分支名> -> [HEAD]，其中HEAD仅、指向当前工作分支），跟踪文件的更改，具体不再多赘述。
...
- 关于权限 -> 除仓库 *owner* 外，要想让其他人拥有仓库上传权限，需将其添加到项目成员目录中，否则只能进行**Pull Request**（Fork、Clone、Push、Pull request、Comment、Merge request）
- 关于分支 -> master分支和main分支是不同时期下的主分支，目前分别被本地git和远程Github作为默认分支，但认为是不同的分支即可；一般来讲，本地仓库和远程仓库的分支名是一一对应的，也可以非对应地提交，但前提是保证不冲突等等
