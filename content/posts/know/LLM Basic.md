...
构建在 LangGraph 运行时之上，中间件架构，LangSmith 用于可观测性
...
def get_current_time(without_seconds: bool = False) -> str:
    ...
    返回:
        当前的日期和时间字符串
    """
    # impportance of docstring for tool description
    from datetime import datetime
...
agent = create_agent(
    ...
    middleware=[
        SummarizationMiddleware(
            ...
            max_tokens_before_summary=2,  # 2 tokens trigger
            # summary_prompt=
        )
    ],
    checkpointer=InMemorySaver(),
...
Load Source File
- import json
- JSONLoader、TextLoader、PyPDFLoader、WebBaseLoader、CSVLoader
"""
# import json
...
# print("documents0] Length", len(documents[0].page_content))
...
- LangGraph 创建带节点和边的状态图 复杂控制流
- Multi-Agent 协调多个专业化 Agent
- **RAG 文档问答系统**
- LangSmith Integration 设置追踪和监控
- Conditional Routing 实现动态工作流路由
- Image Input 使用视觉模型处理图像
- Mixed Modality 结合文本、图像和结构化数据
- File Handling 处理文档上传和分析
- Cost Optimization 追踪 token 使用并优化
